...
class DriverBehaviour(ABC):
    ...
    Instead, other classes inherit from this one and implement the 'decide' method.
    ...
    Method to implement:
        decide(driver: Driver, offer: Offer, time: int) -> bool
            Returns True if the driver accepts the offer, False otherwise.
    """
...
class GreedyDistanceBehaviour(DriverBehaviour):
    ...
    Idea:
        The closer the pickup location is to the driver, the better.
        The driver refuses distant pickups.

    Parameter:
        max_distance (float): Maximum acceptable distance to the pickup location.
    """
...
class EarningsMaxBehaviour(DriverBehaviour):
    ...
    Idea:
        The driver prioritizes earning efficiency rather than distance.
        They evaluate:
            reward per travel time >= threshold
...
class LazyBehaviour(DriverBehaviour):
    """
    Accept an offer only if the driver has been idle for long enough
    AND the pickup is reasonably close.
    ...
    Parameter:
        idle_ticks_needed (int): Minimum number of time ticks the driver must have been idle.
    """
